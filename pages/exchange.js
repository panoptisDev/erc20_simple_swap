...
function Exchange() {
  ...
  const purchase = async () => {
    try {
      ...
      alert("You have successfully purchased JELLY tokens!");
      console.log(request);
    } catch (err) {
      console.error(err);
  ...
  const sell = async () => {
    try {
      ...
      alert("You have successfully sold JELLY tokens!");
      console.log(request);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div>
      <Head>
        <title>Swap JELLY tokens</title>
      </Head>
      <div className="exchange__container">
        
        <h1>Purchase JELLY Tokens</h1>
        <div>0x398e45A300294Fd2e091F3B02fc2CcaEe30db779</div>
        ...
        />
        <div>Equivalent to {matic} MATIC </div> 
